...
Designing the PRBS. Let the monthly-sampled injection-rate perturbation be u[k] added to a baseline $u_0$ (e.g., 5 TgS yr$^{-1}$). We generate a binary sequence s[k] \in {−1,+1} using an m-stage linear-feedback shift register with period N=2^{m}-1 and bit clock T_{b} (nominally one month), then shape it with a stable, linear, time-invariant (LTI) filter h[k] to meet frequency and slew constraints:
